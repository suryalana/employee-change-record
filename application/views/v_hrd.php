...
<body>
			...
			<table id="example" class="table table-striped table-bordered" style="width:100%">
				<thead>
					<div class="position-absolute top-20 end-4">
						<a href="<?= base_url('login'); ?>" class="btn btn-primary " style=" float: right;  margin-top: -20px;">Back</a>
					</div>
				...
				<tbody>
					...
					<tr>
						...
						function checkstatusmanager($d, $id)
						{
							...
							if ($d != 'acc' && $_SESSION['role'] == 'manager') {
								...
								<div id="btnActionMng' . $id . '">
									<button type="button" id="' . $id  . '" class="btn btn-success btnm" style="width"' . $tmp . '>
									...
									<button type="button" id="' . $id  . '" class="btn btn-danger btnrm" ' . $tmp . '>
										<img src="https://img.icons8.com/ios-filled/30/000000/x.png"/>
									</button>
								</div>
								';
							} else {
						...
						<td nowrap align=center>" . checkstatusmanager($i->manager_img, $i->id_erc) . "</td>
	...
	<script>
		$('.btnh').click(function() {
			...
			$.ajax({
				...
				success: function(data) {
					...
					// Reload the page after a successful AJAX call
					location.reload();
				},
		...
		$('.btnm').click(function() {
			var id = $(this).attr('id'); // Get the button ID
			var role = 'manager';
			var type = 'acc';
		...
		$('.btnrm').click(function() {
			...
			$.ajax({
				...
				success: function(data) {
					console.log(data);
					var content = '<p>' + type + '</p>'; // Example of new content
					$('#btnActionMng' + id).html(content);
					location.reload();
				},
				error: function(xhr, status, error) {
					console.error("AJAX Error: ", status, error); // Log any AJAX errors
